...
export default new Elysia({prefix: '/account'})
  ...
  .post('/login-email', async ({set, body, jwt}) => {
    ...
    const response = await sendOfficialEmail(email, accessToken.split('.')[2], 'recover');
  ...
  .post('/signup', async ({set, body, jwt}) => {
    ...
    //@ts-expect-error JWT only accept objects
    const accessToken = await jwt.sign(email + process.env.SECRET_SAUCE);
    const response = await sendOfficialEmail(email, accessToken.split('.')[2], 'confirm');
    if (response.err) return error(set, 500, 'Failed to send verification email. Try later');

    return {email};
  })
  .post('/signup-email', async ({set, body, jwt}) => {
    const {email, access, err} = check(body, ['email', 'access']);
    if (err) return error(set, 400, err);
  ...
  .post('/signup-create', async ({set, jwt, body}) => {
    ...
    //@ts-expect-error JWT only accept objects
    const accessToken = await jwt.sign(email + process.env.SECRET_SAUCE);
    if (access !== accessToken.split('.')[2]) return error(set, 400, 'Invalid access token');
