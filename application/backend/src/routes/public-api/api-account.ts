...
export default new Elysia({prefix: '/account'})
  ...
  .post('/add-account/:id', async ({set, identity, body}) => {
    const fields = ['username', 'password', '?website', '?totp'];
    const {err, username, password, website, totp} = await checkAPI(body, fields);
    ...
    if (totp) await sql`UPDATE accounts SET totp = ${totp!} WHERE id = ${res[0].id!}`;

    return {username, password, website, totp, id: res[0].id};
  })
  .put('/edit-account/:id', async ({set, identity, body}) => {
    const fields = ['username', 'password', '?website', '?totp', 'id'];
    const {err, username, password, website, totp, id} = await checkAPI(body, fields);
    if (err) return error(set, 400, err);
    ...
    if (totp) await sql`UPDATE accounts SET totp = ${totp!} WHERE id = ${id!} AND owner = ${uid}`;

    return {username, password, website, totp, id};
  })
