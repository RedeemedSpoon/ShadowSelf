...
<p>
  The ShadowSelf WebSocket API leverages the WebSocket protocol to establish a persistent, two-way communication channel between your client application and our
  server. Unlike traditional HTTP polling, this allows the server to instantly push notifications about
...
<p>
  We chose WebSockets over alternatives like <b>webhooks</b> primarily for developer convenience and accessibility. Implementing a WebSocket client is straightforward
  in most environments, including browsers and server-side scripts, and crucially, it does not require your application to have a publicly accessible IP address or
  domain name.
</p>
<p>
  To connect, establish a secure WebSocket connection to the bottom endpoint, replacing :id with the specific identity's unique ID. Authentication happens
  during the initial connection handshake using the standard
  <code>Authorization: Bearer YOUR_API_KEY</code> header, identical to the REST API authentication. Once connected (open event), the server begins monitoring; disconnecting
  (close event) stops the monitoring process for that session.
</p>
...
<p>
  To maintain the connection through potential network intermediaries and verify its health, your client must implement a keep-alive mechanism. Periodically
  send a <b>ping</b> message to the server, which will respond with pong if the connection is active. Failure to receive a timely pong likely indicates a dropped
  connection.
</p>
<p>
  All event notifications from the server are delivered as JSON-formatted text messages. Each message object includes a type field that identifies the event,
  along with a corresponding data payload. Your application should parse these messages and route them based on their type. Note that sending messages other
  than "ping" from your client to the server is not part of the intended use and will
  <b>be ignored</b>.
</p>
  ...
  <div>
    ...
    <ul class="mt-4">
      ...
      <ul>
        ...
        <li>
          <code>messageID</code> (<span class="string">string</span>): The globally unique Message-ID header value. Can be empty for drafts.
        </li>
