<script lang="ts">
  ...
  onDestroy(() => {
    if (browser && mediaQueryList && handleChange) {
  ...
  const getGradientAnimateProps = (prefersReduced: boolean) => {
  ...
  const getGradientTransitionProps = (prefersReduced: boolean) => {
    if (prefersReduced) return {};
    return {
  ...
  <svg
    ...
    <defs>
      ...
      <radialGradient
        ...
        <stop offset="0.43594" stop-color="white" stop-opacity="0.2"></stop>
      </radialGradient>
    </defs>
  </svg>
</div>
