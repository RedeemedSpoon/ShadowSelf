<script lang="ts">
  ...
  let {icon, name, type, fill, placeholder, className, handleInput}: Props = $props();
</script>
...
<div class="group flex flex-initial {className?.wrapper}">
  <label title={name} for={name} class={className?.label}>
    {#if icon}
      ...
      <SvelteComponent {fill} className="mx-4 !stroke-neutral-800 !fill-neutral-800 {className?.icon}" />
    {/if}
  </label>
