<script lang="ts">
  ...
  interface Props {
    message: FetchAPI['messages'][number];
    firstime?: boolean;
  }

  let {firstime = true, message}: Props = $props();
...
{#if firstime}
  <div class="my-8 flex w-full items-center justify-center gap-2 text-lg text-neutral-600">
    ...
    <p id="box">
      {formatPhoneNumber(ownMessage ? message.to : message.from)}
    </p>
  </div>
{/if}
<div class:own={ownMessage} class="relative flex justify-between rounded-3xl bg-[#0b1225] px-8 py-6">
  ...
  <div class="h-full w-fit self-center text-sm text-neutral-500">
    {#if new Date(message.date).toLocaleString().split(', ')[0] !== new Date().toLocaleString().split(', ')[0]}
